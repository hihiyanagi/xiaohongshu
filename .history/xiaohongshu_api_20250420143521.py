...
class HtmlParseRequest(BaseModel):
    html_content: str
    use_meta_tags: Optional[bool] = True  # Default to using meta tags for direct HTML input
...
def parse_html_content(soup: BeautifulSoup, use_meta_tags: bool = False, base_url: str = None):
    ...
    if use_meta_tags:
        ...
        # 提取图片 - 从og:image meta标签 (可能使用name或property属性)
...
async def scrape_article(request: ScrapeRequest):
    try:
        ...
        # 解析HTML内容
        title, content, images = parse_html_content(soup, request.use_meta_tags, request.url)
...
@app.post("/parse-html", response_model=ScrapeResponse)
async def parse_html(request: HtmlParseRequest):
    try:
        ...
        # 解析HTML内容
        title, content, images = parse_html_content(soup, request.use_meta_tags)
        ...
        if not content and not images:
            return ScrapeResponse(
                ...
                images=images,
                success=False,
                message="No content found in the provided HTML."
            )
        ...
        return ScrapeResponse(
            ...
            images=images,
            success=True,
            message="Content successfully parsed from HTML"
        )
...
async def root():
    return {
        ...
        "endpoints": {
            ...
            "/parse-html": "POST endpoint for parsing article content from HTML",
