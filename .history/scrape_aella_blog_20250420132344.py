...
def scrape_aella_blog():
    ...
    headers = {
        ...
        'DNT': '1',
        'Referer': 'https://aella.substack.com/',
    }
    ...
    try:
        ...
        # Debug: Print all divs with their IDs
        print("\nDebug: All divs with IDs:")
        for div in soup.find_all('div', id=True):
            print(f"Found div with id: {div['id']}")
        
        # Find the entry div
        entry_div = soup.find('div', id='entry')
        if not entry_div:
            print("\nDebug: Could not find the entry div. Trying alternative methods...")
            
            # Try finding by class name
            entry_div = soup.find('div', class_='entry')
            if not entry_div:
                print("Could not find div with class 'entry'")
                
                # Try finding by data attributes
                entry_div = soup.find('div', attrs={'data-entry': True})
                if not entry_div:
                    print("Could not find div with data-entry attribute")
                    
                    # Try finding by content structure
                    entry_div = soup.find('div', class_='post-content')
                    if not entry_div:
                        print("Could not find div with class 'post-content'")
                        return
                    else:
                        print("Found content using class 'post-content'")
                else:
                    print("Found content using data-entry attribute")
            else:
                print("Found content using class 'entry'")
        else:
            print("Found content using id 'entry'")
            
        # Find the main content div within entry
        main_div = entry_div.find('div', id='main')
        if not main_div:
            print("\nDebug: Could not find the main div. Trying alternative methods...")
            main_div = entry_div.find('div', class_='main')
            if not main_div:
                print("Could not find div with class 'main'")
                main_div = entry_div  # Use entry_div as fallback
            else:
                print("Found content using class 'main'")
        else:
            print("Found content using id 'main'")
        ...
        if len(full_content.strip()) < 100:
            print("\nWarning: Very little content was retrieved. The content might be behind a paywall.")
            print("Debug: Content length:", len(full_content.strip()))
        
        # Write to file
        with open('blog_content.txt', 'w', encoding='utf-8') as f:
            f.write(full_content)
        
        print("\nContent has been successfully scraped and saved to 'blog_content.txt'")
